...
export RE10K_USE_SCENE_POSES=1
...
### LOG DIR ###
LOG_DIR="${CODE}/scripts/logs/re10k_dense/use_scene_poses/${OUT_NAME}"
...
# OUTPUT_DIR="${DEBUG}/re10k_dense_use_scene_poses/${OUT_NAME}"
OUTPUT_DIR="${WORK}/data/${OUT_NAME}"
...
FLAGS: 
    - RE10K_USE_SCENE_POSES=1
    - DA3_LOG_LEVEL=DEBUG
LOG_DIR: "$LOG_DIR"
EOF

# change permission:
echo -e "${LOG_MSG} CHANGING PERMISSION OF ${OUTPUT_DIR} TO: g+rwx"
echo -e "${LOG_MSG} BEFORE: $(ls -ld ${OUTPUT_DIR})"
chmod -R g+rwx "${OUTPUT_DIR}"
echo -e "${LOG_MSG} AFTER: $(ls -ld ${OUTPUT_DIR})"
