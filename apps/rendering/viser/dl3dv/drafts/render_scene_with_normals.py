...
def load_depth_map(depth_path: Path) -> Optional[np.ndarray]:
    if not depth_path.exists():
        debug_print(1, f"Path does not exist: {depth_path}")
        return None
    
    ext = depth_path.suffix.lower()
    
    if ext == '.exr':
        return _load_exr(depth_path)
    elif ext == '.png':
        return _load_png(depth_path)
    elif ext == '.npy':
        return _load_npy(depth_path)
    
    debug_print(1, f"Unsupported extension: {ext}")
    return None

def _load_exr(depth_path: Path) -> Optional[np.ndarray]:
    try:
        import OpenEXR
        import Imath
        ...
        for channel in ['R', 'Y', 'Z', 'G']:
            try:
                depth_str = exr_file.channel(channel, Imath.PixelType(Imath.PixelType.FLOAT))
                depth = np.frombuffer(depth_str, dtype=np.float32).reshape((height, width))
                debug_print(1, f"Loaded EXR via OpenEXR (Channel {channel}): {depth.shape}")
                return depth
            except: continue
    except ImportError:
        debug_print(2, "OpenEXR not installed, trying imageio...")
    except Exception: pass

    for loader_name in ['imageio.v2', 'imageio']:
        try:
            import imageio
            depth = imageio.imread(str(depth_path))
            if depth is not None:
                if depth.ndim == 3: depth = depth[:, :, 0]
                debug_print(1, f"Loaded EXR via {loader_name}: {depth.shape}")
    ...
    try:
        ...
        if depth is not None:
            if depth.ndim == 3: depth = depth[:, :, 0]
            debug_print(1, f"Loaded EXR via 'exr' package: {depth.shape}")
            return depth.astype(np.float32)
    except Exception as e:
        debug_print(1, f"Final EXR attempt failed: {e}")

    return None

def _load_png(depth_path: Path) -> Optional[np.ndarray]:
    depth = cv2.imread(str(depth_path), cv2.IMREAD_ANYDEPTH | cv2.IMREAD_UNCHANGED)
    if depth is not None:
        debug_print(1, f"Loaded PNG: {depth.shape}")
        return depth.astype(np.float32)
    return None

def _load_npy(depth_path: Path) -> Optional[np.ndarray]:
    """Load NumPy array depth maps."""
    try:
        depth = np.load(str(depth_path))
        debug_print(1, f"Loaded NPY: {depth.shape}")
        return depth
    except Exception as e:
        debug_print(1, f"Error loading NPY: {e}")
        return None

def find_depth_file(dataset_root: Path, image_path: Path, dataset: str) -> Optional[Path]:
    stem = image_path.stem
    
    depth_dir = dataset_root / dataset / "v0" / "depth"
    if not depth_dir.exists():
        debug_print(2, f"Depth directory not found: [depth_dir={depth_dir}].")
        return None
    
    for ext in ['.exr', '.png', '.npy']:
        depth_path = depth_dir / f"{stem}{ext}"
        if depth_path.exists():
            debug_print(2, f"Found depth file: [depth_path={depth_path}].")
            return depth_path
    
    debug_print(2, f"No depth file found for [image_path.stem={image_path.stem}].")
    return None


def find_normal_file(dataset_root: Path, image_path: Path, dataset: str) -> Optional[Path]:
    stem = image_path.stem
    
    normals_dir = dataset_root / dataset / "v0" / "normals"
    if not normals_dir.exists():
        debug_print(1, f"Normals directory not found: [normals_dir={normals_dir}].")
        return None
    
    for ext in ['.exr', '.png', '.npy']:
        normal_path = normals_dir / f"{stem}{ext}"
        if normal_path.exists():
            debug_print(2, f"Found normal file: [normal_path={normal_path}].")
            return normal_path
    
    debug_print(2, f"No normal file found for [image_path.stem={image_path.stem}].")
    return None
...
def get_parser():
    ...
    parser.add_argument("--render_normals", action="store_true",
                        help="Enable normals visualization (default: False)")
    parser.add_argument("--dataset", type=str, default="mvsanywhere", choices=["mvsanywhere", "moge"],
                        help="Dataset to use (default: mvsanywhere)")
    return parser
...
def main():
    ...
    for i, frame in enumerate(frames[::args.frame_skip]):
        ...
        depth_path = find_depth_file(dataset_root, image_path, args.dataset)
